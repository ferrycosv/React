      ...
      <form>
        <div className="field">
          <input
            ...
            onChange={(event) => {
              setNumberA(Number(event.target.value));
            }}
          ...
          <input
            ...
            onChange={(event) => {
              setNumberB(Number(event.target.value));
            }}
            /* add an attribute with value here that causes numberA to update when the input changes */
          />
        </div>
...
const FruitForm = () => {
  ...
  let error = ["banana", "apple"].includes(fruit) ? false : true; /* Add an expression here that validates if fruit is a banana or apple */
