  ...
  render() {
    return (
      <section className="contact">
        ...
        {this.state.submittedText && <p>{this.state.submittedText}</p>}
      </section>
    );
  }
}
...
class FruitForm extends React.Component {
  ...
  onChange = (event) => {
    this.setFruit(event.target.value);
  };

  render() {
    const error = ["apple", "banana"].includes(this.state.fruit)
      ? false
      : true; /* Add an expression here that validates if fruit is a banana or apple */
    return (
      <section className="contact">
        <form onSubmit={this.onSubmit}>
          <div className="field">
